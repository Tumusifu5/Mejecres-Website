...
$stmt = $conn->prepare("SELECT id, fullname, reset_expires FROM teachers WHERE reset_token = ?");
...
if (strtotime($teacher['reset_expires']) < time()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($password) || empty($confirm_password)) {
        ...
        $update_stmt = $conn->prepare("UPDATE teachers SET password = ?, reset_token = NULL, reset_expires = NULL WHERE id = ?");
        $update_stmt->bind_param("si", $hashed_password, $teacher['id']);

        if ($update_stmt->execute()) {
            $message = '✅ Your password has been reset successfully. You can now <a href="teacher-login.php">login</a>.';
...
<title>Reset Password - Teacher - MEJECRES SCHOOL</title>
